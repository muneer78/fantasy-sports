...
df_kcsandlotkeepers["Group"] = df_kcsandlotkeepers["Service Time"].apply(
    determine_group
)
...
df1 = df_kcsandlotkeepers.merge(
    df_draftsheet[["Key", "LRank", "ADP", "Team", "Total Z-Score"]],
    on=["Key"],
    how="left",
)
...
df1.sort_values(
    by=["Group", "LRank", "Total Z-Score"], ascending=[True, True, False], inplace=True
)
