...
dfopponents = pd.read_csv('FanGraphs Leaderboard(10).csv', usecols=["Team", "K%", "BB%"])
...
dfsched = pd.merge(dfsched, dfopponents[['Team', 'KRank', 'BBRank']], on='Team', how='left')
...
# matching_dfbbtop25 = dfsched[(dfsched['Name'].isin(dfbbtop25['Name'])) & ((dfsched['KRank'] <= 10) | (dfsched['BBRank'] <= 10))]
matching_dfbbtop25 = dfsched[(dfsched['Name'].isin(dfbbtop25['Name'])) & ((dfsched['BBRank'] <= 10))]
matching_dfktop25 = dfsched[(dfsched['Name'].isin(dfktop25['Name'])) & ((dfsched['KRank'] <= 10))]
...
titles = ["Pitcher High Strikeout Props", "Pitcher Low BB Props", ]
