...
sheets_credentials = ServiceAccountCredentials.from_json_keyfile_name(
    "laghezza-e6374719f2f8.json", scope_sheets
)
...
sheet = gc_sheets.open_by_url(
    "https://docs.google.com/spreadsheets/d/1sjfqRGZxpQ9xkwpc6WrPMdsJgcIOfFHWGNgnkW0ebCc/"
)
...
def join_fields(df, l4_df, l8_df, pos, output_file, title):
    if pos in df["Pos"].unique():
        ...
        merged_df = df.merge(l4_df, on="Player").merge(
            l8_df, on="Player", suffixes=("_L4", "_L8")
        )
...
for df in dataframes_list:
    if "Player" in df.columns and "Fantasy Pts" in df.columns:
        if "QB" in df["Pos"].unique():
            ...
            join_fields(
                df, l4_receiving_df, l8_receiving_df, "WR", output_file, "WR/TE"
            )
