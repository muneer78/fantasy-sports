...
weekly_rec_stats = (
    pbp.query("play_type == 'pass'")
    .groupby(["game_id", "receiver_player_id", "receiver_player_name"])
    .agg(targets=("play_id", "count"), catches=("complete_pass", "sum"))
    .reset_index()
    .rename(
        columns={
...
weekly_stats_player = pd.merge(weekly_rec_stats, weekly_rush_stats, how="outer").fillna(
    0
)

# calculate touches
weekly_stats_player["touches"] = (
    weekly_stats_player["catches"] + weekly_stats_player["carries"]
)

# now get total number of offensive plays (pass or run) per team and game
team_total_plays = (
    pbp.query("(play_type == 'pass') | (play_type == 'run')")
...
weekly_stats_player = pd.merge(
    weekly_stats_player, games[["game_id", "season", "week"]]
)
...
weekly_stats["rec_market_share"] = (
    weekly_stats["targets"] / weekly_stats["team_pass_attempts"]
)


# look at the results
rec_market_share_wk12 = weekly_stats.query(
    "(week == 12) & ((position == 'WR') | (position == 'TE'))"
)[["player_name", "position", "rec_market_share"]].sort_values(
    "rec_market_share", ascending=False
)

rb_market_share_wk12 = weekly_stats.query("(week == 12) & (position == 'RB')")[
    ["player_name", "rb_market_share"]
].sort_values("rb_market_share", ascending=False)

rb_market_share_all = weekly_stats.query("(position == 'RB')")[
    ["player_name", "week", "rb_market_share"]
].sort_values("rb_market_share", ascending=False)

rec_market_share_all = weekly_stats.query("(position == 'WR') | (position == 'TE')")[
...
# main plot
g = sns.relplot(
    x="week", y="rec_market_share", kind="line", aspect=1.2, data=rec_market_share_all
)
# add title
...
g = sns.relplot(
    ...
    hue="position",
    aspect=1.2,
    data=rec_market_share_all.groupby(["week", "position"]).max().reset_index(),
)
g.fig.subplots_adjust(top=0.9)
...
top_rb = (
    rb_market_share_all.groupby("player_name")
...
g = sns.relplot(
    ...
    height=2,
    aspect=1.2,
    data=pd.merge(rb_market_share_all, top_rb),
    legend=False,
)
g.fig.subplots_adjust(top=0.9)
