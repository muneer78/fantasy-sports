...
count_A_pitchers = df_kcsandlotkeepers[
    (df_kcsandlotkeepers["Group"] == "A")
    & (
        (df_kcsandlotkeepers["Position"] == "SP")
        | (df_kcsandlotkeepers["Position"] == "RP")
    )
].shape[0]
...
if count_A_pitchers > 4:
    messages.append(
        f"You have {count_A_pitchers} pitchers in group A. You can have up to 4"
    )
...
pivot_table = pd.pivot_table(
    df_kcsandlotkeepers, values=["Name"], index=["Group"], aggfunc="count"
)
